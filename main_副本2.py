...
if __name__ == '__main__':
    ...
    for i in range(1,count):
        if ws1.cell(row=i+1,column=1).value==ws2.cell(row=i+1,column=1).value:
            sheet9.cell(row=j + 1, column=1).value = ws1.cell(row=i+1,column=1).value
            sheet9.cell(row=j+1, column=2).value = int(ws1.cell(row=i+1,column=2).value) + int(ws2.cell(row=i+1,column=2).value)
            j = j+1
        else:
            if ws1.cell(row=i+1,column=2).value !=None:
                sheet9.cell(row=j + 1, column=1).value = ws1.cell(row=i + 1, column=1).value
                sheet9.cell(row=j+1, column=2).value = int(ws1.cell(row=i+1,column=2).value)
                j = j + 1
            if ws2.cell(row=i+1,column=2).value!=None:
                sheet9.cell(row=j + 1, column=1).value = ws2.cell(row=i + 1, column=1).value
                sheet9.cell(row=j+1, column=2).value = int(ws2.cell(row=i+1, column=2).value)
                j = j + 1
